...
**Live:** [districtflow-safety.vercel.app](https://districtflow-safety.vercel.app)
...
# 6. Deploy (see Deployment below for why this is a CLI step, not git-triggered)
vercel --prod --yes --project districtflow-safety
```
...
segments, so deployment needs no live backend/server.

## Deployment

**Live:** [districtflow-safety.vercel.app](https://districtflow-safety.vercel.app)

Deployed via the Vercel CLI (`vercel --prod`) run from `dashboard/` with the
real, generated `dashboard/public/data/*.json` present on disk.
**Deliberately NOT connected to git-triggered auto-deploy**: those JSON/
GeoJSON files are real model output (5,744 segments' worth), regenerated by
step 4 above, and correctly `.gitignore`d — the repo commits the *pipeline
that produces them*, not the (large, model-run-specific) output itself, same
convention as `data/raw`/`data/processed`. A git-triggered build would clone
the repo without them and ship an empty, broken dashboard. Re-deploying
after a real pipeline re-run is therefore: re-run steps 1-4, then
`vercel --prod --yes --project districtflow-safety` again from `dashboard/`.
